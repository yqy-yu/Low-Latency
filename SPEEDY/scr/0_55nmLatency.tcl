...
set PDKDIR [format "%s%s"  [getenv "UMC55"] "/"]
set search_path [concat  $search_path $PDKDIR]
set alib_library_analysis_path $PDKDIR
set libname "fsf0a_ers_core_ff1p1vm40c.ccdb"
set link_library [set target_library [concat  [list $libname] [list dw_foundation.sldb]]]
set target_library $libname



analyze -library WORK -format vhdl -autoread {../verilog}
elaborate cipher_top -architecture Behavioral -library WORK

set_max_delay -from tx_in -to text_out  0
set_max_delay -from key_in -to text_out  0
...
uplevel #0 { report_timing -path full -delay max -nworst 40 -max_paths 40 -significant_digits 2 -sort_by group > 0_55nm_minlatency_timing_lp.txt}

uplevel #0 { report_area -hierarchy > 0_55nm_minlatency_area_lp.txt}
